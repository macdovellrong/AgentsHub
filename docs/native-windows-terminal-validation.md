...
源码版脚本会依次执行 PowerShell/Codex 预检、cmd/Codex 预检、`scrolltest` 预检，并生成诊断报告。发布包版脚本位于 `artifacts/native/win-x64/validate-native-laptop.ps1`，会检查包文件、PowerShell/cmd host、Codex native launcher 和 `--no-alt-screen`，再生成诊断报告。即使某个预检失败，也会继续生成报告；最近一次报告路径会写入 `artifacts/native-diagnostics/latest-laptop-validation.txt`。
