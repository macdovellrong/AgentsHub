...
5. 启动 Codex、Claude、Gemini 或普通 shell session；也可以用 `Start Agents` 按 Codex、Claude、Gemini 顺序一次启动当前 workspace 的三类托管 Agent，或用 `Resume Agents` 以 `codex resume` + Claude/Gemini 普通启动的组合恢复协作环境，并在状态栏显示成功启动数量。
