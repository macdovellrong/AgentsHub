...
16. manager hook 输出的 `assign_task` / `request_review` / `reject_task` / `approve_task` / `pause_plan` 在完成现有终端转发后，会同步写入执行快照内的 `tasks.jsonl` / `events.jsonl`；完整状态机仍在迁移中。
17. 可以用 Interrupt 向当前 session 发送 Ctrl+C 而不关闭终端；用 Stop selected 停止当前 session，也可以用 Stop all 停止全部 session。
