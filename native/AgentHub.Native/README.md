...
11. Agent hook 回传如果包含 `<agenthub>{"action":"send_message","to":"codex","message":"..."}</agenthub>` 或旧格式 `<agenthub>{"action":"send","target":"codex","task_id":"T-001","message":"..."}</agenthub>`，native app 会按当前 workspace 路由到目标 profile 的最新 session。
